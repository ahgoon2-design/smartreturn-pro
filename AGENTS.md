...
## 위험도 라벨 (개발속도 정책)
모든 작업카드는 착수 전 위험도 라벨 1개를 단다. 라벨은 새 공정을 만들지 않고 아래 기존 규칙을 호출하는 키워드다. 개별 지시문에 규칙 전문을 반복하지 않고 라벨로 대체한다.

🟢 문서전용
- 대상: 문서·오타·화면문구·주석·테스트명 (코드·DB·권한·테넌시·반품/재고/정산·secret 무관)
- 규칙: 수정 → 선별 add → 커밋 → push 한 번에. diff에 코드/.env/secret 혼입 0 확인이 전제. 별도 커밋·push 승인 없음.

🟡 조회/UI
- 대상: read-only API, 단순조회 화면, UI 컴포넌트, 테스트 보강, 동작 불변 리팩터
- 규칙: 스펙 승인 → 구현 → 독립검수 🟢 → 수문장 커밋 → push gate 3종 통과 후 push. 구현자 ≠ 독립검수자.

🔴 코드/DB/권한/테넌시
- 대상: 인증, 권한·역할, tenant scope, schema, migration, 반품상태, 입출고 경계, ownership, 개인정보·운송장·정산
- 규칙: 🟡 전부 + SPEC 승인 필수(스펙 없는 구현 금지) + 고위험 서버가드 4계약(필드/enum·실패응답·불변조건·음성테스트) 충족. 검수 🟢 + 신규회귀 0이면 push까지 자동.

⚫ 최고위험 (테넌트격리·중앙락·재고반영·반품확정·입출고확정·멱등·상태전이·정산확정·운영DB write)
- 규칙: 🔴 전부 + push 전 사장님에게 신호등 보고 + 타 대리점(타 agency) 누수 0 증거 제시 → 사장님 커밋 승인 후 수문장 커밋·push.

절대 (라벨 무관, 불변):
- 구현 주체 ≠ 독립검수·커밋 주체
- git add . 금지 / 선별 add only
- secret·.env·평문 비밀번호·토큰은 어떤 문서·커밋·보고에도 비기재
- 검수 통과 전 commit 금지

작업카드 필수필드: 작업명 / 위험도(🟢🟡🔴⚫ 중 1) / 수정 허용 / 수정 금지 / 구현자 / 독립검수자 / 커밋 주체 / push 조건 / 필수 검증 / 다음 1수.
...
## Karpathy 보강분 A~D (코드 작업 행동 규칙)

### 참조 규약
- 개별 지시문은 본문에 규칙을 복붙하지 않고 "Karpathy 보강분 A~D 적용" 한 줄로 참조한다.
- 우선순위: 개별 지시문이 특정 원칙을 명시적으로 완화하면 그 지시문이 우선한다. 명시적 완화가 없으면 A~D가 항상 적용된다.
- 사소한 작업 예외: 오타·명백한 한 줄 수정 등 trivial 작업은 절차를 완화할 수 있다. 단 ⚫ 최고위험 지시문은 예외 없이 전부 적용한다.

### 적용 주체
- Codex(구현자): 구현 중 A~D를 준수한다.
- Claude Code(독립검수·수문장): A~D를 검수 체크리스트로 쓴다. 위반 시 게이트 통과를 거부한다.
- 이 절은 기존 규칙(역할 분리, git 스테이징, 진입/중단 조건, §9 테스트 계약, 보고 형식)에 없던 "코드를 어떻게 쓰고 검토하는가"만 보강한다. 겹치는 항목은 반복하지 않고 참조한다.

### A. 가정하지 말고 드러낸다
기존 중단조건은 하드 블로커(저장소 불일치, OVER_REVIEW 강제 등)만 잡는다. 이 원칙은 그보다 약한 "모호함"을 처리한다. 비대면 실행이므로 되묻는 대신 아래 3갈래로 처리한다:
- 하드 블로커 → 기존 중단조건대로 중단.
- 정답을 못 정할 만큼 모호 → 중단하고 보고에 무엇이 불명확한지 명시(중단조건 확장).
- 안전한 기본값이 있는 모호 → 그 기본값으로 진행하되 보고에 "~라고 가정하고 구현함"으로 명시하고 검수 대상으로 표시.
- 해석이 갈리면 조용히 하나를 고르지 않는다. 더 단순한 방법이 있으면 진행 전에 제안한다. SPEC과 dirty 구현이 충돌하면 임의 봉합하지 말고 표면화한다.
- 검수(CC): 근거 없는 암묵 가정이 보고에 있으면 반려.

### B. 최소 구현
SPEC 보완은 요청된 범위만 만든다.
- 요청되지 않은 기능·설정·유연성 추가 금지. 1회용 코드에 추상화 금지. 일어날 수 없는 시나리오의 방어 코드 금지.
- 200줄이 50줄로 되면 다시 쓴다. 판단 기준: "시니어가 보면 과한가?" → 그렇다면 단순화.
- 최적화 취급(각주): 측정된 병목이나 SPEC 명시 요구가 없는 선제 최적화(캐시·인덱스·추상화)는 이 원칙 위반으로 본다. 다만 트랜잭션·row-level lock·재고 정합처럼 정확성·동시성에 직결되는 것은 "최적화"가 아니라 요건이며 SPEC 잠금 기준(예: SPEC-005 잠금 8·9)을 따른다. 성능 우려를 발견하면 조용히 고치지 말고 보고에 적고 별도 SPEC으로 분리 제안한다.
- 검수(CC): SPEC 요구를 넘는 투기적 코드나 근거 없는 성능용 구조가 있으면 반려.

### C. 최소 침습 변경
기존 "파일별 스테이징 / 수정 허용·금지 범위"는 어떤 파일을 건드리냐를 통제한다. 이 원칙은 허용된 파일 안에서 어떤 줄을 건드리냐를 통제한다(두 규칙은 짝).
- 작업과 무관한 인접 코드·주석·서식을 "개선"하지 않는다. 안 깨진 코드를 리팩터하지 않는다. 내 취향과 달라도 기존 스타일을 따른다.
- 내 변경 때문에 미사용이 된 import·변수·함수만 정리한다. 원래 있던 죽은 코드는 지시 없이 건드리지 않는다.
- 무관한 죽은 코드를 발견하면 삭제하지 말고 보고서 "남은 위험/미해결" 칸에만 적는다.
- 판단 기준: 바뀐 모든 줄이 SPEC 보완 항목으로 추적돼야 한다.
- 검수(CC): diff에 SPEC과 무관한 변경(서식·리팩터·인접 개선)이 섞이면 반려.

### D. 목표 기반 검증 (순서 보강)
기존 §9 테스트 계약·R1–R9 회귀와 겹치되, 작성 순서만 보강한다(테스트 항목 자체는 각 SPEC의 §9를 따른다).
- 버그 수정은 재현 실패 테스트를 먼저 쓰고 통과시킨다. 리팩터는 전후 동일 테스트 통과를 보인다.
- "동작하게 만들어라" 같은 약한 기준 대신 검증 가능한 성공 기준으로 바꿔 루프한다.
- Codex는 성공 기준을 명시하고 그 기준까지 자체 루프한다. 단 커밋·push는 하지 않는다(기존 규칙 유지).
