...
## 언어 규칙
- 모든 응답, 설명, 보고, 주석은 한국어로 작성한다.
- 코드 식별자(변수명·함수명·클래스명), DB 컬럼명, API path, enum 값, 파일명은 영어를 유지한다.
- 사용자가 한국어로 질문하면 반드시 한국어로 답한다.
- 영어로 질문해도 한국어로 답한다.
...
- 대리점 SaaS 범위의 업무 데이터는 `agency_id` scope를 함께 지킨다.
...
- 반품판정은 고객사별 판정표/체크리스트/과거 확정 이력 기반으로 추천할 수 있지만 최종 확정은 작업자가 한다.
- 사진/영상 AI 자동판별은 1차 목표가 아니며 사진은 증빙/분쟁 대응 중심이다.
- 반품처리 화면은 세부항목 없는 반품도 상품 스캔/검색/선택으로 처리할 수 있어야 한다.
- 반품 판정 즉시 현재고를 변경하지 않고, `warehouse_id`가 확정되지 않은 판정은 처리완료하지 않는다.
...
> Smart AI Dev Harness용 Claude Code 운영 가이드. 위의 SmartReturn Pro 작업 기준을 우선하며, 충돌 시 더 안전한 규칙을 따른다.

## 1. Startup

1. `<PROJECT_ROOT>/AGENTS.md`를 읽는다.
2. `<PROJECT_ROOT>/CLAUDE.md`를 읽는다.
3. 사용자 지시에서 작업 모드를 판단한다.
4. `.claude/agents/` 전체를 한 번에 읽지 않는다.
5. 주관 agent 하나를 선택한다.
6. 선택한 agent의 참조 문서만 읽는다.
7. 필요 시 handoff 규칙에 따라 다음 agent로 넘긴다.

## 2. Claude Agent Selection

- 설계/범위/우선순위 판단: `smartreturn-pm`
- 구조/DB/권한/테넌시 영향 검토: `smartreturn-architect`
- API/service/test/backend: `backend-engineer`
- React/UI/CSS/frontend: `frontend-engineer`
- 그리드/스크롤/복사/loading/UX: `ux-grid-specialist`
- 엑셀/CSV/구글시트/import: `import-mapper-specialist`
- 테스트/빌드/diff check: `qa-tester`
- secret/권한/파괴적 작업 위험: `security-guard`
- 최종 보고: `report-writer`

## 3. Handoff Rule

- 구현 agent는 작업 완료 후 `qa-tester`로 넘긴다.
- UI/Grid 변경은 `ux-grid-specialist` 검수 후 `qa-tester`로 넘긴다.
- 검증 완료 후 `report-writer`가 보고서를 정리한다.
- 위험 발견 시 즉시 `security-guard`로 넘긴다.
