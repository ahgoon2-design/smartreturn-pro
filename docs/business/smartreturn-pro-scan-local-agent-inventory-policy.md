...
- 반품 판정/처리완료 즉시 `current_inventory`를 변경하지 않는다.
- 반품 재고는 일마감 또는 반출/폐기 확정 후 `agency_id`, `client_id`, `client_unit_id`, `warehouse_id`, `product_id`, `judgment_code` 기준으로 반영한다.
...
- 스캔 처리와 그리드 선택 처리가 서로 다른 검증 경로로 갈라지지 않았는가?
