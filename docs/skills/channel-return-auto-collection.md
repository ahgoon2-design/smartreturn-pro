---
name: channel-return-auto-collection
description: >-
  네이버/쿠팡/카페24/이지어드민/택배사 API 반품 자동수집 작업 시 적용하는 채널 API 반품
  자동수집 스킬. 외부 채널 원본 수집, canonical 정규화, 중복 upsert, 예외상태 분리, 현장
  스캔 연결, 채널 역전송 단계화 기준을 정리하므로 외부 채널 반품 자동수집 작업 시 반드시 이
  스킬을 적용한다.
---
