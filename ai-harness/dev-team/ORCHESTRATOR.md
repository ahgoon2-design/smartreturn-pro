...
* 커밋 후보 선별
* git 안전 확인
* 사용자가 명시적으로 수정 작업으로 전환한 경우에만 코드 수정
* 독립 검수 모드에서는 코드 수정 금지, 수정 필요 항목을 보고하고 구현 담당으로 되돌림
...
똘순이 인수인계가 있을 때 개발팀은 아래 항목이 있는지 확인한다.
...
위 항목이 없거나 모호하면 바로 구현하지 않고 보류/질문으로 되돌린다.
